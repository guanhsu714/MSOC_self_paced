...
int main()
{
	...
	std::ifstream golden("D:/Faker/hls/fft/fft/solution1/csim/build/out.fft.gold.dat");
	...
	for(int i=0; i<SIZE; i++) {
	  ...
	  int j;
	  golden >> j >> rx >> ix;
	  DTYPE newerror = fab(rx-In_R[i]) + fab(ix-In_I[i]);
	  error += newerror;
	...
	if ((error/SIZE) > 0.05 || maxerror > 2) { // This is somewhat arbitrary.  Should do proper error analysis.
	  fprintf(stdout, "*******************************************\n");
